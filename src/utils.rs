...
pub fn address_pair_symbol(address1: ResourceAddress, address2: ResourceAddress) -> String {
    ...
    let names: (String, String) = (
        match borrow_resource_manager!(addresses.0).metadata().get_string("symbol") {
            Ok(s) => {
                format!("{}", s)
            },
            Err(_e) => {
                format!("{:?} has no symbol", addresses.0)
            }
        },
        match borrow_resource_manager!(addresses.1).metadata().get_string("symbol") {
            Ok(s) => {
                format!("{}", s)
            },
            Err(_e) => {
